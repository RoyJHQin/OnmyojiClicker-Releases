...
## ⚠️ OCR 识别与设备 / 窗口分辨率重要说明

> [!WARNING]
> **不同设备、系统显示缩放、安卓模拟器分辨率及游戏窗口大小，都可能造成 OCR 识别结果差异。** 这可能进一步导致界面识别错误或无法识别，以及部分依赖文字识别的按钮出现点击定位错误或无法定位。
>
> **请根据自己的实际设备和运行环境进行测试，并按实际识别结果调整界面识别、文字条件及点击配置。** 默认配置只能作为起始配置，不能保证在所有设备、模拟器和窗口尺寸下得到完全一致的识别结果。
>
> **原开发与主要测试环境：13.6 英寸 MacBook Air（M5），macOS 默认显示设置（默认分辨率 / 缩放）。** 正式连续运行前，请先使用“只观察”和“单步点击”核查当前设备上的界面识别与文字按钮定位是否准确。
